...
declare capture_time timestamptz; v_target_id uuid := coalesce(new.visible_to_player_id,new.excluded_player_id);
begin
  if new.type='attack' or (new.type='warning' and new.message='คุณถูกโจมตีและเสียหัวใจ 1 ดวง') then
    if v_target_id is not null then
      if to_regclass('public.v24_actions') is not null then
        select e.captured_at into capture_time from public.evidence e
         left join public.v24_actions va on va.evidence_id=e.id
         where e.room_id=new.room_id and e.target_id=v_target_id and e.status in ('pending','approved')
         order by (e.status='pending') desc,case when e.status='pending' then va.effective_at end asc nulls last,
                  e.decision_at desc nulls first,e.created_at desc,e.id desc limit 1;
      else
        select e.captured_at into capture_time from public.evidence e
         where e.room_id=new.room_id and e.target_id=v_target_id and e.status in ('pending','approved')
         order by (e.status='pending') desc,e.decision_at desc nulls first,e.created_at desc,e.id desc limit 1;
      end if;
    else
      if to_regclass('public.v24_actions') is not null then
        select e.captured_at into capture_time from public.evidence e
         left join public.v24_actions va on va.evidence_id=e.id
         where e.room_id=new.room_id and e.status in ('pending','approved')
         order by (e.status='pending') desc,case when e.status='pending' then va.effective_at end asc nulls last,
                  e.decision_at desc nulls first,e.created_at desc,e.id desc limit 1;
      else
        select e.captured_at into capture_time from public.evidence e
    ...
    if capture_time is not null then new.created_at:=capture_time; end if;
  end if;
  return new;
end $$;
drop trigger if exists attack_event_capture_time on public.room_events;
create trigger attack_event_capture_time before insert on public.room_events
