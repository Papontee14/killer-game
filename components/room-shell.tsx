...
export function PlayerRoom({ code, name = readRoomCredentials(`player:${code}`)?.name }: { code: string; name?: string }) {
  ...
  const capture = async () => { try { const live = await captureLivePhoto(); setPhoto(live.dataUrl); setCapturedAt(live.capturedAt); } catch (e) { setError(errorMessage(e, "เปิดกล้องไม่สำเร็จ")); } };
  const choosePhoto = (_file: File | undefined) => setError("การส่งหลักฐานต้องใช้กล้องสด");
