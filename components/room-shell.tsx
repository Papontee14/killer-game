...
export function HostRoom({ code, name }: { code: string; name?: string }) {
  ...
  const act = (operation: Promise<RoomState>) => { setError(""); void run(operation).catch((e) => setError(errorMessage(e, "ดำเนินการไม่สำเร็จ"))); };
...
export function PlayerRoom({ code, name = readRoomCredentials(`player:${code}`)?.name }: { code: string; name?: string }) {
  ...
  const [reclaimToken, setReclaimToken] = useState(remembered?.reclaimToken || "");
  ...
  const join = async (event?: React.FormEvent) => { event?.preventDefault(); setJoining(true); try { const joined = await joinOrCreateDemo(code, loginName, reclaimToken); const token = joined.reclaimToken || reclaimToken; rememberRoomCredentials(`player:${code}`, { name: loginName, reclaimToken: token || undefined }); setReclaimToken(token); if (joined.reclaimToken) window.alert(`บันทึกรหัสกู้คืนนี้ไว้สำหรับเปลี่ยนอุปกรณ์: ${joined.reclaimToken}`); setPlayerId(joined.playerId); setRoom(joined.room); } catch (e) { setError(errorMessage(e, "เข้าห้องไม่ได้")); } finally { setJoining(false); } };
  ...
  if (!name || (!playerId && !room && !joining)) return <main className="landing-shell"><section className="access-panel"><div className="access-heading"><span>ROOM ACCESS</span><h2>เข้าห้อง</h2><p>ใช้รหัสห้องและชื่อผู้เล่นเพื่อเข้าร่วม; หากเปลี่ยนอุปกรณ์ ให้ใส่รหัสกู้คืนของผู้เล่น</p></div><form onSubmit={join}><label>ชื่อผู้เล่น<input required value={loginName} onChange={(e) => setLoginName(e.target.value.slice(0, 24))} /></label><label>รหัสกู้คืน (เฉพาะอุปกรณ์ใหม่)<input value={reclaimToken} onChange={(e) => setReclaimToken(e.target.value.replace(/[^a-f0-9]/gi, "").slice(0, 32))} autoCapitalize="off" /></label><button className="primary-action" disabled={joining}>เข้าห้อง <ArrowFallback /></button></form><ErrorBanner error={error} /></section></main>;
