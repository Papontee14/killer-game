...
import { database, fixture, migration, schema } from './db-harness.mjs';
...
test('all nine roles have correct hearts; role pool rejects invalid setup and Host cannot play',async()=>{
  for(const [role,hearts] of Object.entries({killer:0,'killer-wife':2,police:2,reporter:2,bomber:2,detective:2,athlete:3,sumo:4,villager:2})) {
  ...
  for (const counts of [null,{}, {killer:0,police:1,villager:8},{killer:2,police:1,villager:6},{killer:1,police:0,villager:8},{killer:1,police:1,villager:7,reporter:null}])
    await assert.rejects(rpc('host','start_game',counts));
  const started=await rpc('host','start_game',{killer:1,police:1,athlete:1,sumo:1,villager:5});
...
test('pending/rejected evidence does no damage, warning or quota; approved nonlethal attack does all three privately',async()=>{
  ...
  assert.equal((await view('villager')).events.filter(e=>e.type==='warning').length,0);
  assert.equal((await view('host')).attacksThisHour,0);
  await rpc('host','reject_evidence',id);
  assert.equal((await f.state('villager')).hearts,2);
  await f.hit('villager');
  const mine=await view('villager'), other=await view('athlete'), killer=await view('killer');
  assert.equal(mine.privateStates[f.players.villager].hearts,1);
  assert.equal(mine.events.filter(e=>e.playerId===f.players.villager&&e.type==='warning').length,1);
  assert.equal(other.players.find(p=>p.id===f.players.villager).health,'alive');
  assert.equal(killer.players.find(p=>p.id===f.players.villager).health,'alive');
  assert.equal(killer.killerEvidenceProgress.find(e=>e.status==='approved').result,'target is still alive');
...
test('quota is shared, counts nonlethal attacks and resets by Bangkok calendar hour',async()=>{
  await f.hit('sumo'); await f.hit('athlete');
  const id=await f.evidence('sumo'); // full quota still permits submission
  ...
  const bucket=(await db.query("select quota_window_start=(date_trunc('hour',clock_timestamp() at time zone 'Asia/Bangkok') at time zone 'Asia/Bangkok') as ok from public.rooms")).rows[0];
  assert.equal(bucket.ok,true);
  const boundaries=(await db.query(`select date_trunc('hour','2026-09-05 08:59:59+07'::timestamptz at time zone 'Asia/Bangkok') as before,
    date_trunc('hour','2026-09-05 09:00:00+07'::timestamptz at time zone 'Asia/Bangkok') as after`)).rows[0];
...
test('wife transforms privately, both Killers see shared progress, quota grows without reset',async()=>{
  await f.hit('killer-wife'); await f.hit('killer-wife');
  const wife=await view('killer-wife'), killer=await view('killer'), city=await view('villager');
  assert.equal(wife.privateStates[f.players['killer-wife']].isActiveKiller,true);
  assert.equal(wife.privateStates[f.players['killer-wife']].maxHearts,0);
  assert.equal(wife.attackLimit,3); assert.equal(wife.attacksThisHour,2);
  ...
  assert.equal(city.players.find(p=>p.id===f.players['killer-wife']).health,'alive');
  const publicAbility=city.events.filter(e=>e.type==='ability');
  assert.deepEqual(publicAbility.map(e=>e.message),["Killer has eliminated Killer's Wife. There are now two Killers."]);
  assert.equal(wife.events.filter(e=>e.playerId===f.players['killer-wife']&&e.type==='warning').length,2);
  ...
  for(const progress of (await view('killer')).killerEvidenceProgress)
    for(const key of ['storagePath','imageData','hearts','maxHearts','initialRole','currentRole']) assert.equal(key in progress,false);
...
test('normal lethal attack sends a private warning and generic public death',async()=>{
  ...
  assert.equal(dead.events.filter(e=>e.playerId===f.players.villager&&e.type==='warning').length,2);
  assert.equal((await view('killer')).killerEvidenceProgress.filter(e=>e.result==='elimination confirmed').length,1);
});

test('reporter refuses self/dead/NULL targets without consuming ability and reads initial role once',async()=>{
  for(const target of [f.players.reporter,null]) await assert.rejects(rpc('reporter','use_reporter',target));
  ...
  await f.hit('killer-wife'); await f.hit('killer-wife');
  const result=await rpc('reporter','use_reporter',f.players['killer-wife']);
  assert.ok(result.events.some(e=>e.playerId===f.players.reporter&&e.message.endsWith('killer-wife')));
  await assert.rejects(rpc('reporter','use_reporter',f.players.killer));
  const target=await view('killer-wife');
  assert.ok(target.events.some(e=>e.message==='คุณถูกตรวจบทบาท'&&e.playerId===f.players['killer-wife']));
  assert.ok(!(await view('athlete')).events.some(e=>e.message.includes('บทบาทเริ่มต้น')));
});
...
test('promoted detective still reports initial Detective; reporter is allowed during accusation and bomb phase',async()=>{
  ...
  await db.exec("update public.player_secrets set has_used_ability=false where role_current='reporter'; update public.rooms set phase='bomb-resolution'");
...
test('Host, outsider, wrong role, dead actor and unauthenticated caller cannot use role RPCs',async()=>{
  for(const role of ['host','outsider','villager','nobody']) await assert.rejects(rpc(role,'use_reporter',f.players.killer));
  assert.equal((await view('villager')).events.some(e=>e.message.includes('บทบาทเริ่มต้น')),false);
  await db.query("update public.players set health='dead' where id=$1",[f.players.reporter]);
  await assert.rejects(rpc('reporter','use_reporter',f.players.killer));
  await db.exec("update public.rooms set phase='police-check'");
  for(const role of ['host','outsider','villager','nobody']) await assert.rejects(rpc(role,'resolve_police_check',f.players.killer));
  for(const id of [null,f.players.police]) await assert.rejects(rpc('police','resolve_police_check',id));
...
test('cross-room target and Host-only mutations refuse outsiders',async()=>{
  ...
  await assert.rejects(rpc('killer','submit_evidence',joined.playerId,'missing',new Date().toISOString()));
  for(const [fn,args] of [['start_game',[{}]],['approve_evidence',[null]],['reject_evidence',[null]],['resolve_bomb',[[]]],['set_accusation_at',[new Date().toISOString()]],['close_room',[]]])
...
test('heartbeat only changes presence for the authenticated member; internal helpers are not callable',async()=>{
  for(const role of ['host','outsider','nobody']) await assert.rejects(rpc(role,'heartbeat'));
  await rpc('villager','heartbeat');
  for(const name of ['advance_due_accusation','room_for_code','add_event']) {
    const args=name==='advance_due_accusation'?[f.roomId]:name==='room_for_code'?['ABCDEF']:[f.roomId,'ability','injected',null];
...
test('upgrade and fresh-install definitions remain identical for all migrated RPCs',()=>{
  const definitions=text=>new Map([...text.matchAll(/create or replace function public\.(\w+)\([\s\S]*?\$\$;/g)].map(match=>[match[1],match[0]]));
  const fresh=definitions(schema);
  for(const [name,body] of definitions(migration)) assert.equal(body,fresh.get(name),name);
});
...
test('deadline is persisted by direct approval without damage and by direct submission without evidence',async()=>{
  ...
  await db.exec("update public.rooms set police_check_at=clock_timestamp()-interval '1 second'");
  const result=await rpc('host','approve_evidence',id);
  assert.equal(result.actionError,'accusation_started'); assert.equal(result.phase,'police-check');
  ...
  assert.equal((await db.query('select count(*)::int n from public.evidence')).rows[0].n,1);
...
test('freshness applies at submission only; pending evidence never expires on Host review',async()=>{
  ...
  const id=await f.evidence('sumo');
  await db.query("update public.evidence set captured_at=captured_at-interval '1 day',created_at=created_at-interval '1 day' where id=$1",[id]);
  await rpc('host','approve_evidence',id);
  assert.equal((await f.state('sumo')).hearts,3);
});
...
test('Bomber auto-pauses attacks and reveals only its own role',async()=>{
  ...
  await f.hit('bomber'); const result=await f.hit('bomber');
  assert.equal(result.phase,'bomb-resolution');
  assert.ok(result.events.some(e=>e.type==='bomb'&&e.message.endsWith('Bomber')));
...
for(const targets of [['killer','police'],['police','killer']]) test('last Killer and Police explosion: city wins regardless of order '+targets.join(','),async()=>{
  await db.query("update public.players set health='dead' where id=$1",[f.players.detective]);
  const result=await f.bomb(targets);
  assert.equal(result.winner,'city'); assert.equal(result.pendingBomberId,null);
  assert.ok(result.events.filter(e=>e.type==='bomb').every(e=>!e.message.includes('Killer')));
});

for(const targets of [['police','detective'],['detective','police']]) test('Police and Detective explosion: Killers win regardless of order '+targets.join(','),async()=>{
...
test('bomb accepts zero victims, refuses duplicates/NULL/too many/dead victims and no wife transformation',async()=>{
  ...
  for(const ids of [null,[null],[f.players.killer,f.players.killer],Object.values(f.players).slice(0,3),[f.players.bomber]])
...
test('bomb kills transformed Killer; old pending attacks of a dead Killer cannot be approved',async()=>{
  await f.hit('killer-wife'); await f.hit('killer-wife'); await f.resetQuota();
  const id=await f.evidence('sumo','killer-wife');
  const result=await f.bomb(['killer-wife']);
  assert.equal(result.winner,null);
  await assert.rejects(rpc('host','approve_evidence',id),/killer is not active/);
  assert.equal((await f.state('sumo')).hearts,4);
});
...
test('bomb ends in accusation if deadline passed; Police accusation decides either winner',async()=>{
  ...
  assert.equal((await rpc('police','resolve_police_check',f.players.killer)).winner,'city');
  await db.exec("update public.rooms set phase='police-check',winner=null");
  assert.equal((await rpc('police','resolve_police_check',f.players.sumo)).winner,'killers');
});

test('Detective death alone continues, Police later dies without successor: Killers win',async()=>{
  await f.hit('detective'); await f.hit('detective');
  assert.equal((await view('host')).winner,null);
  await f.resetQuota(); await f.hit('police');
...
test('Storage reads are Host-only and Realtime signals remain authorized without table grants',async()=>{
  ...
  for(const role of ['killer','reporter','host','outsider']) {
    ...
    try {
      assert.equal((await db.query('select * from storage.objects')).rows.length,role==='host'?1:0);
      assert.equal((await db.query('select * from public.room_signals')).rows.length,role==='outsider'?0:1);
      await assert.rejects(db.query('select * from public.player_secrets'),/permission denied/);
    } finally { await db.exec('reset role'); }
  }
});

test('migration upgrades an active room without changing gameplay data; legacy results stay unknown',async()=>{
  await f.hit('sumo'); const pending=await f.evidence('athlete');
  ...
  const snapshot=async()=>Object.fromEntries(await Promise.all(['rooms','players','player_secrets','evidence','room_events'].map(async table=>[table,(await db.query(`select * from public.${table} order by 1`)).rows])));
  const before=await snapshot();
  await db.exec(migration); await db.exec(migration); // repeat is safe
  const after=await snapshot();
  for(const e of after.evidence) { assert.equal(e.attack_result,null); delete e.attack_result; }
