...
export function LandingGame() {
  ...
  function continueToRoom(event: React.FormEvent) {
    ...
    const params = new URLSearchParams({ name: name.trim() || (mode === "host" ? "Host" : "ผู้เล่น"), pin: pin || "1234" });
    router.push(mode === "host" ? `/room/${makeRoomCode()}/host?${params}` : `/room/${roomCode}?${params}`);
  }
